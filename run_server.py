...
def check_frontend_submodule(lang=None):
    ...
    if not frontend_path.exists():
        ...
        try:
            ...
            logger.critical(
                f'Failed to initialize submodules: {e}.\nThere might be a network issue between you and GitHub. You might see {{"detail":"Not Found"}} in your browser. Please check our quick start guide and common issues page from our documentation.\n'
            )


def parse_args():
    parser = argparse.ArgumentParser(description="RemAI Server")
...
def run(console_log_level: str):
    ...
    logger.info(f"RemAI, version v{get_version()}")
