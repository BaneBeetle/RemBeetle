...
TEXTS = {
    "zh": {
        ...
        "not_git_repo": "错误：当前目录不是git仓库。请进入 RemAI 目录后再运行此脚本。\n当然，更有可能的是你下载的RemAI不包含.git文件夹 (如果你是透过下载压缩包而非使用 git clone 命令下载的话可能会造成这种情况)，这种情况下目前无法用脚本升级。",
...
是否继续？(y/N): """,
    ...
    "en": {
        # "welcome_message": f"Auto-Upgrade Script {CURRENT_SCRIPT_VERSION}\nRemAI upgrade script - This script is highly experimental and may not work as expected.",
        ...
        "not_git_repo": "Error: Current directory is not a git repository. Please run this script inside the RemAI directory.\nAlternatively, it is likely that the RemAI you downloaded does not contain the .git folder (this can happen if you downloaded a zip archive instead of using git clone), in which case you cannot upgrade using this script.",
